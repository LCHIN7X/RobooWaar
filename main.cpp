...
public:
    ...
    // overload << operator to accept generic data types
    template <typename T>
...
public:
    ...
    void takeDamage()
    {
        if (!hidden)
        {
            ...
            if (battlefield)
                battlefield->queueForReentry(this); // Immediately queue for reentry
        }
    }
...
void GenericRobot::act() // when it's a robot's turn, this function is first called
{
