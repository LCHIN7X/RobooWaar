...
void GenericRobot::fire(int X, int Y)
{
    ...
    if (hasAmmo())
    {
        // Filter detectedTargets to only include robots that are hittable (canBeHit() == true) and not already hurt
        vector<Robot *> validTargets;
        for (Robot *r : detectedTargets)
        {
            //GG
            if ((r && r != this) && !r->getIsHurt()) // <-- Only allow not hurt robots
            {
    ...
    void fire(int X, int Y) override
    {
        ...
        for (int dx = -3; dx <= 3; dx++)
        {
            for (int dy = -3; dy <= 3; dy++)
            {
                ...
                if (target && target != this && !target->getIsHurt()) // <-- Only allow not hurt robots
                {
...
{
    // Only queue if not already queued this round
...
void Battlefield::respawnRobots()
{
    if (!reentryQueue.empty())
    {
        ...
        if (spotFound)
        {
            ...
            if (newShooter) {
                //GG  DAMN
                newShooter->setAmmo(ammoLeft); // Set the preserved ammo
            }
        ...
        else
        {
            ...
            //GG
            // If no spot, requeue for next turn with correct ammo
...
void Battlefield::cleanupDestroyedRobots()
{
    ...
    while (iterator != listOfRobots.end())
    {
        ...
        //GG
        // If robot is hurt but not dead, queue for reentry before deleting
